...
These are Python-side aliases for the CSS custom properties defined in
`assets/purch_theme.css`. Keeping them here means components can reference
palette tokens by name (e.g. `COLORS['coral']`) instead of hard-coding hex
values everywhere, and any future palette change lives in one file plus
the CSS.
"""
...
# Reusable Tailwind class fragments for the common surfaces. Components
# compose these instead of re-typing long class strings.
...
# navigation links. Kept alongside the theme so it's trivial to find.
ROUTES: dict[str, str] = {
    "index": "/",
    "login": "/login",
    "chat": "/chat",
