"""Chat bubble component — mirrors the Streamlit treatment.

Assistant bubbles pick up a coloured border when the message carries a
budget alert (warning = gold, danger = red), matching the alert
convention used in `app.py::classify_alert`.
"""

import reflex as rx

from app.app.states.chat_state import ChatMessage
...
def typing_indicator() -> rx.Component:
    """Animated three-dot indicator shown while the (currently stubbed,
    later real) agent is generating a reply."""
    dot_class = (
    ...
    return rx.el.div(
        ...
        rx.el.div(
            ...
            rx.el.div(
                class_name=f"{dot_class} [animation-delay:150ms]",
            ),
            rx.el.div(
                class_name=f"{dot_class} [animation-delay:300ms]",
            ),
            class_name=(
                "purch-bubble-assistant flex items-center gap-1.5 py-3"
            ),
        ),
